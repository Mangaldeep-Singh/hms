...
from wtformsparsleyjs import IntegerField,FloatField, BooleanField, SelectField, StringField, PasswordField, TextAreaField, RadioField, DateField
...
class CreateCustomer(FlaskForm):
    ...
    bed = SelectField(u'Type of Bed', choices=[('general', 'Genral Ward'), ('semi', 'Semi Sharing'), ('single', 'Single Room')])
    address = StringField("Address", [ validators.DataRequired(message="Address is Required") ])
    state = StringField("State",[ validators.DataRequired(message="Address is Required") ])
    city = StringField("city",[ validators.DataRequired(message="Address is Required") ])

class CreateMedicine(FlaskForm):
    name = StringField("Name", [validators.DataRequired(message="Name is Required"), validators.Regexp(message='Name should only alphabet', regex=r'^[a-zA-Z ]*$') , validators.Length(message='Name should only alphabet and minimum 4 character', min=4, max=25) ] )
    quantity = IntegerField("Quantity", [validators.DataRequired(message="Quantity is Required"), validators.NumberRange(message='Minimum Quantity is 10', min=10, max=120) ])
    rate = IntegerField("Rate", [ validators.DataRequired(message="Rate is Required"), validators.NumberRange(message='Minimum Rate is 10', min=10, max=9999) ])
    
class CreateDiagnostic(FlaskForm):
    name = StringField("Name", [validators.DataRequired(message="Name is Required"), validators.Regexp(message='Name should only alphabet', regex=r'^[a-zA-Z ]*$') , validators.Length(message='Name should only alphabet and minimum 4 character', min=4, max=25) ] )
    rate = IntegerField("Rate", [ validators.DataRequired(message="Rate is Required"), validators.NumberRange(message='Minimum Rate is 10', min=10, max=9999) ])
...
class SearchCustomer(FlaskForm):
    search = IntegerField("Patient ID", [validators.DataRequired(message="Patient ID is Required"), validators.NumberRange(message='SSN should be exactly 9', min=100000000, max=999999999) ])
    searchname= StringField("Name", [validators.DataRequired(message="Name is Required"), validators.Regexp(message='Name should only alphabet', regex=r'^[a-zA-Z ]*$') , validators.Length(message='Name should only alphabet and minimum 4 character', min=4, max=25) ] )
